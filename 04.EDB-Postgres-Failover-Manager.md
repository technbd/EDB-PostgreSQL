...
Let’s simulate a crash of our primary database by killing the corresponding process.
...
## Recover (192.168.10.192):

_Remove old data directory:_
```
cd /var/lib/edb/as16/data
rm -rf *
```

_Take fresh base backup from PRIMARY (192.168.10.191):_
```
/usr/edb/as16/bin/pg_basebackup -h 192.168.10.191 -p 5444 -U replicator -D /var/lib/edb/as16/data -Fp -Xs -P -R

Or,

/usr/edb/as16/bin/pg_basebackup -h 192.168.10.191 -p 5444 -U replicator -D /var/lib/edb/as16/data -Fp -Xs -P -R -C -S standbyslot1
```


_Delete the line `primary_conninfo = 'user=replicator password=replicator channel_binding=prefer host=192.168.10.192` :_
- It MUST contain only ONE primary (PostgreSQL allows ONLY ONE `primary_conninfo`)
- No duplicate lines
- No reference to 192.168.10.192 itself
...
primary_conninfo = 'user=replicator password=replicator channel_binding=prefer host=192.168.10.191 port=5444 sslmode=prefer sslcompression=0 sslcertmode=allow sslsni=1 ssl_min_protocol_version=TLSv1.2 gssencmode=prefer krbsrvname=postgres gssdelegation=0 target_session_attrs=any load_balance_hosts=disable'
```
